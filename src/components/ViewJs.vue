<template>
    <div class="VJS_video-player-wrapper" v-on:keydown.space="togglePlayPause" tabindex="0">
        ...
        <video class="VJS_video-player" ref="videoPlayer" v-on:click="togglePlayPause" :preload="props.preload">
            <source v-for="source in props.sources" :src="source.url" />
            Your browser does not support HTML5 video.
        </video>
        <div class="VJS_controls" ref="videoControls" v-bind:class="{ 'VJS_controls--fixed': !videoBeingPlayed }">
            <button class="VJS_controls__button" v-on:click="togglePlayPause" v-bind:class="{
            ...
            <div class="VJS_controls__bar">
                <div ref="progressBar" class="VJS_controls__progress" v-on:click="skipToPosition($event)">
                    ...
                    <span class="VJS_controls__progress-back" v-bind:class="{ 'VJS_started': percentagePlayed !== 0 }"
                        v-bind:style="{ width: percentagePlayed }"></span>
                    <div class="VJS_controls__progress-ranges">
                        <span class="VJS_controls__progress-range" v-for="range in bufferRanges"
                            v-bind:style="{ left: range.start, width: range.end }"></span>
                    </div>
                </div>
                <div class="VJS_controls__time">{{ timeRemaining }}</div>
                <div class="VJS_controls__volume">
                    <div class="VJS_controls__volume-bar"
                        v-bind:class="{ 'VJS_controls__volume-bar--active': volume >= 0.2 }"
                        v-on:click="adjustVolume(0.2)"></div>
                    <div class="VJS_controls__volume-bar"
                        v-bind:class="{ 'VJS_controls__volume-bar--active': volume >= 0.4 }"
                        v-on:click="adjustVolume(0.4)"></div>
                    <div class="VJS_controls__volume-bar"
                        v-bind:class="{ 'VJS_controls__volume-bar--active': volume >= 0.6 }"
                        v-on:click="adjustVolume(0.6)"></div>
                    <div class="VJS_controls__volume-bar"
                        v-bind:class="{ 'VJS_controls__volume-bar--active': volume >= 0.8 }"
                        v-on:click="adjustVolume(0.8)"></div>
                    <div class="VJS_controls__volume-bar"
                        v-bind:class="{ 'VJS_controls__volume-bar--active': volume === 1 }"
                        v-on:click="adjustVolume(1)"></div>
                </div>
...
.VJS_controls__volume-bar:first-child {
    left: 0;
    height: 5px;
}

.VJS_controls__volume-bar:nth-child(2) {
    left: 5px;
    height: 8px;
}

.VJS_controls__volume-bar:nth-child(3) {
    left: 10px;
    height: 12px;
}

.VJS_controls__volume-bar:nth-child(4) {
    left: 15px;
    height: 15px;
}

.VJS_controls__volume-bar:last-child {
    left: 20px;
    height: 18px;
}

.VJS_controls__volume-bar:first-child:hover {
    height: 8px;
}

.VJS_controls__volume-bar:nth-child(2):hover {
    height: 11px;
}

.VJS_controls__volume-bar:nth-child(3):hover {
    height: 15px;
}

.VJS_controls__volume-bar:nth-child(4):hover {
    height: 18px;
}

.VJS_controls__volume-bar:last-child:hover {
    height: 21px;
}
